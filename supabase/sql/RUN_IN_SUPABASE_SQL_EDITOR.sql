...
-- ===============================================================
-- Push delivery pipeline foundations
-- Source: 013_push_delivery_pipeline.sql
-- ===============================================================
...
-- Push sanity checks
-- ------------------------------------------------------------------
-- 1) Register token from app login flow, then confirm token row exists:
-- select user_id, token, platform, is_active, last_seen_at
-- from public.device_tokens
-- order by updated_at desc
-- limit 20;

-- 2) Insert a notification and verify queue rows are created:
-- insert into public.notifications (user_id, type, title, body)
-- values ('PASTE_USER_ID'::uuid, 'direct_message', 'Test Push', 'If push pipeline is healthy, a queue job should appear.');
--
-- select id, notification_id, user_id, status, token, created_at
-- from public.notification_push_jobs
-- order by created_at desc
-- limit 20;

-- 3) Delivery worker note:
-- A worker/edge function must consume pending jobs and call FCM/APNs.
-- Jobs are generated automatically by trigger on public.notifications inserts.

-- ------------------------------------------------------------------
-- Rollback companions (manual)
-- ------------------------------------------------------------------
-- hardening rollback: supabase/sql/012_shops_fields_hardening_rollback.sql
-- push rollback:      supabase/sql/013_push_delivery_pipeline_rollback.sql
